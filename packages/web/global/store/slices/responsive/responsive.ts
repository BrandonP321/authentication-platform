...
class MediaQuery {
	...
	constructor(maxWidth: number) {
		...
		this._query = typeof window !== undefined ? window.matchMedia(`screen and (max-width: ${maxWidth}px)`) : null;
	}

	public testQuery = () => {
		return this._query?.matches ?? false;
	}
...
export class Responsive {
	...
	private static checkMediaQueries = () => {
		...
		for (queryKey in this.queries) {
			...
			const matches = mq.query?.matches ?? false;
